...
HANDLE                      g_hBeginPerChunkRenderDeferredEvent[g_iMaxPerChunkRenderThreads];
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"MultithreadedRendering11 - RemoveSemaphores&Dummy" );
...
HRESULT InitializeWorkerThreads( ID3D11Device* pd3dDevice )
{
    ...
    for ( int iInstance = 0; iInstance < g_iNumPerChunkRenderThreads; ++iInstance )
    {
		...
		g_hBeginPerChunkRenderDeferredEvent[iInstance] = CreateEvent(NULL, FALSE, FALSE, NULL);
    ...
    if ( IsRenderDeferredPerChunk() )
    {
        if ( IsRenderMultithreadedPerChunk() )
        {
            ...
            for ( int iInstance = 0; iInstance < g_iNumPerChunkRenderThreads; ++iInstance )
            {
                ...
                //ReleaseSemaphore( hSemaphore, 1, NULL );
				SetEvent(g_hBeginPerChunkRenderDeferredEvent[iInstance]);
            }
...
unsigned int WINAPI _PerChunkRenderDeferredProc( LPVOID lpParameter )
{
    ...
    for (;;)
    {
		...
		WaitForSingleObject(g_hBeginPerChunkRenderDeferredEvent[iInstance], INFINITE);
		bool bEnd = FALSE;
		while (!bEnd)
		{
...
void CALLBACK OnD3D11DestroyDevice( void* pUserContext )
{
    ...
    for ( int iInstance = 0; iInstance < g_iNumPerChunkRenderThreads; ++iInstance )
    {
		...
		CloseHandle(g_hBeginPerChunkRenderDeferredEvent[iInstance]);
